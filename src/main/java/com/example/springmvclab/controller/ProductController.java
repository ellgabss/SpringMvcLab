...
public class ProductController {
    ...
    @GetMapping("/categories") // Membuat alamat baru: /products/categories
    public String showCategorySummary(Model model) {
        // Mengambil data kategori dari service yang sudah kita update tadi
        model.addAttribute("categories", productService.getAllCategories());

        // Mengirim 'service' agar kita bisa menghitung jumlah produk di HTML nanti
        model.addAttribute("service", productService);
        model.addAttribute("title", "Ringkasan Kategori");

        return "product/categories"; // Menuju ke folder templates/product/categories.html
    }
}